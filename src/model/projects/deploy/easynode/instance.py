    ...
    async def deploy_contract(self):
        try:
            ...
            if receipt["status"] == 1:
                ...
                logger.success(
                    f"{self.account_index} | Hợp đồng Cpunter triển khai thành công! Address: {contract_address} TX: {EXPLORER_URL_MEGAETH}{tx_hex}"
                )
            ...
            logger.error(
                f"{self.account_index} | Lỗi triển khai Hợp đồng đối ứng: {e}. Waiting {random_pause} seconds..."
            )
