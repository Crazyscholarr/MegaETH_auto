        ...
        try:
           ...
           # Chờ nhận giao dịch
            ...
            if receipt["status"] == 1:
                ...
                logger.success(
                    f"{self.account_index} | Hợp đồng Owlto được triển khai thành công! Address: {contract_address} TX: {EXPLORER_URL_MEGAETH}{tx_hex}"
                )
            ...
            logger.error(
                f"{self.account_index} |Lỗi triển khai hợp đồng Owlto: {e}. Đang chờ {random_pause} seconds..."
            )
