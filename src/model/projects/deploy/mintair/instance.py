import asyncio
import random
import hashlib
import time
import os

from eth_account import Account
from src.model.onchain.web3_custom import Web3Custom
from loguru import logger
import primp
from web3 import Web3
from src.utils.decorators import retry_async
from src.utils.config import Config
from src.utils.constants import EXPLORER_URL_MEGAETH

CHAIN_ID = 6342  # Từ nhận xét constants.py

TIMER_PAYLOAD = ""


class Mintair:
    def __init__(
        self,
        account_index: int,
        session: primp.AsyncClient,
        web3: Web3Custom,
        config: Config,
        wallet: Account,
    ):
        self.account_index = account_index
        self.session = session
        self.web3 = web3
        self.config = config
        self.wallet = wallet

    @retry_async(default_value=False)
    async def deploy_timer_contract(self):
        try:
            # Chuẩn bị giao dịch cơ bản
            tx = {
                "from": self.wallet.address,
                "to": "", # Địa chỉ trống để triển khai hợp đồng
                "data": TIMER_PAYLOAD,
                "value": 0,
            }

            # Ước tính khí
            try:
                gas_limit = await self.web3.estimate_gas(tx)
                tx["gas"] = gas_limit
            except Exception as e:
                raise e

            # Nhận thông số giá gas
            gas_params = await self.web3.get_gas_params()
            tx.update(gas_params)

            # Thêm ID chuỗi và nonce
            tx["chainId"] = CHAIN_ID
            tx["nonce"] = await self.web3.web3.eth.get_transaction_count(
                self.wallet.address
            )

           # Ký giao dịch
            signed_tx = self.web3.web3.eth.account.sign_transaction(tx, self.wallet.key)

            # Gửi giao dịch
            tx_hash = await self.web3.web3.eth.send_raw_transaction(
                signed_tx.raw_transaction
            )
            tx_hex = tx_hash.hex()

            # Chờ nhận giao dịch
            receipt = await self.web3.web3.eth.wait_for_transaction_receipt(tx_hash)

            if receipt["status"] == 1:
                contract_address = receipt["contractAddress"]
                logger.success(
                    f"{self.account_index} | Hợp đồng hẹn giờ được triển khai thành công! Address: {contract_address} TX: {EXPLORER_URL_MEGAETH}{tx_hex}"
                )
                return contract_address
            else:
                raise Exception(f"Giao dịch không thành công. Tình trạng: {receipt['status']}")

        except Exception as e:
            random_pause = random.randint(
                self.config.SETTINGS.PAUSE_BETWEEN_ATTEMPTS[0],
                self.config.SETTINGS.PAUSE_BETWEEN_ATTEMPTS[1],
            )
            logger.error(
                f"{self.account_index} | Lỗi triển khai hợp đồng Timer: {e}. Đang chờ {random_pause} seconds..."
            )
            await asyncio.sleep(random_pause)
            raise
