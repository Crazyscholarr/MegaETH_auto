...
class OmniHub:
    ...
    async def mint(self):
        try:
            ...
            if not contracts_to_mint:
                logger.error(
                    f"{self.account_index} | Không tìm thấy hợp đồng để đúc"
                )
                return False
            ...
            logger.info(
                f"{self.account_index} | Đúc NFT từ {contract_to_mint['title']} với giá cả {contract_to_mint['price']}"
            )

            # Check current NFT balance before minting
            current_balance = await self._check_nft_balance(contract_to_mint['address'])
            logger.info(
                f"{self.account_index} | Số dư NFT hiện tại cho hợp đồng {contract_to_mint['address']}: {current_balance}"
            )
            ...
            logger.error(
                f"{self.account_index} | Lỗi đúc OmniHub: {e}. Chờ đợi {random_pause} seconds..."
            )
    ...
    async def _check_nft_balance(self, contract_address: str) -> int:
        """Kiểm tra số dư NFT cho ví hiện tại từ hợp đồng đã cho."""
        try:
            ...
            logger.error(f"{self.account_index} | Lỗi kiểm tra số dư NFT: {e}")
            raise e
    ...
    async def _mint_nft(self, contract: dict):
        try:
            ...
            if receipt["status"] == 1:
                logger.success(
                    f"{self.account_index} | Đúc thành công OmniHub NFT tại {title}: {EXPLORER_URL_MEGAETH}{tx_hash}"
                )
                return True
            else:
                logger.error(
                    f"{self.account_index} | Giao dịch không thành công: {EXPLORER_URL_MEGAETH}{tx_hash}"
                )
                return False
        ...
        while page < 6:
            for retry in range(3):
                try:
                    ...
                    logger.error(
                        f"{self.account_index} | Lỗi nhận token ngẫu nhiên cho mint: {str(e)}"
                    )
                    if retry == 2:
                        return constracts
