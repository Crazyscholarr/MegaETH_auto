...
class GteFaucet:
    ...
    async def faucet(self):
        try:
            ...
            if receipt["status"] == 1:
                logger.success(
                    f"{self.account_index} | GTE Faucet được yêu cầu thành công! TX: {EXPLORER_URL_MEGAETH}{tx_hex}"
                )
                return True
            else:
                raise Exception(
            ...
            logger.error(
                f"{self.account_index} | Lỗi yêu cầu GTE Faucet: {e}. Chờ đợi  {random_pause} seconds..."
            )
