...
class OnchainGm:
    ...
    async def GM(self):
        try:
            ...
            if receipt["status"] == 1:
                logger.success(
                    f"{self.account_index} | GM trên OnchainGM thành công! TX: {EXPLORER_URL_MEGAETH}{tx_hex}"
                )
                return True
            else:
                logger.error(
                    f"{self.account_index} | Giao dịch không thành công. Tình trạng: {receipt['status']}"
                )
                raise Exception(
            ...
            logger.error(
                f"{self.account_index} | Lỗi GM trên OnchainGM: {e}. Đang chờ {random_pause} seconds..."
            )
