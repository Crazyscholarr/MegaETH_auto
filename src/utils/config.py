...
class WithdrawalConfig:
    ...
    max_wait_time: int
    retries: int
    max_balance: float  # Số dư ví tối đa để cho phép rút tiền


@dataclass
class ExchangesConfig:
    name: str  # Tên sàn giao dịch (OKX, BINANCE, BYBIT)
    apiKey: str
    secretKey: str
    passphrase: str  # Chỉ cần cho OKX
    ...
    def load(cls, path: str = "config.yaml") -> "Config":
        ...
        try:
            ...
            if hasattr(tasks, "TASKS"):
                ...
                error_msg = "Không tìm thấy danh sách TASKS trong tasks.py"
                print(f"Lỗi: {error_msg}")
                raise ValueError(error_msg)
        except ImportError as e:
            error_msg = f"Không thể nhập tasks.py: {e}"
            print(f"Lỗi: {error_msg}")
